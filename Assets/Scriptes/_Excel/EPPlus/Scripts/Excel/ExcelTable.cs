...
public class ExcelTable
{
    ...
    public ExcelTable(ExcelWorksheet sheet)
    {
        ...
        for (int row = 1; row <= NumberOfRows; row++)
        {
            ...
            {
                string value = GetSheetRowColVal(sheet,row, column);
    ...
    public string GetSheetRowColVal(ExcelWorksheet sheet,int row,int column)
    {
        try
        {
        ...
        {
            if (isDebugError) {
                Debug.LogError("sheet name = " + sheet.Name+",row = " + row + ",column = " + column + " = error = " + ex);
            }
        }
        return "";
    }
